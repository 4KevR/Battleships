...
public class MousePlaceShip implements EventHandler<MouseEvent> {
    ...
    public MousePlaceShip (ResizableCanvas canvasHoverPlaceShip, int shipLength, Player player) {
        this.player = player;
        this.canvasHoverPlaceShip = canvasHoverPlaceShip;
        this.hover = this.canvasHoverPlaceShip.getGraphicsContext2D();
        this.shipLength[0] = shipLength; this.shipLength[1] = 1;
        this.canvasHoverPlaceShip.setOnMouseClicked(mouseEvent -> {
            if (mouseEvent.getButton() == MouseButton.SECONDARY) {
                ...
                hover.clearRect(0, 0, this.canvasHoverPlaceShip.getWidth(), this.canvasHoverPlaceShip.getHeight());
    ...
    public void handle (MouseEvent ev){
        ...
        double width = canvasHoverPlaceShip.getWidth();
        double height = canvasHoverPlaceShip.getHeight();

        hover.clearRect(0, 0, canvasHoverPlaceShip.getWidth(), canvasHoverPlaceShip.getHeight());

        if (x > (120.0/1920)*width && x < (840.0/1920)*width) {
            int [] temp = new int[2];
